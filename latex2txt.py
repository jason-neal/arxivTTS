...
def get_arxiv_src(arxivname):
    ...
    # remove text file
    subprocess.call("mv {0} {1}/{0}".format(filename, "TMP"), shell=True)
...
def main(arxivID, output=False, ext="mp3", player="mplayer", saveAudio=True,
         saveText=False, keepSrc=False, keepTex=False, autoplay=False,
         options=""):
    ...
    if not saveAudio:
        if autoplay:
            ...
            subprocess.call("festival --tts {0}/{1} {2}".format(FINALDIR,
                            output_txt, options), shell=True)  # rm text file
        else:
        ...
        start = time.time()
        print("Saving audio ...")
        subprocess.call("text2wave {0}/{1} -o {0}/{2}".format(FINALDIR,
                        output_txt, output_audio), shell=True)
        print("Finished saving to arXiv:{2} to {0}/{1}".format(FINALDIR,
              output_audio, srcname))
        ...
        # ##### Google TTS
        # import GoogleTTS
        # GoogleTTS.audio_extract(input_text='tunnel snakes rule apparently',
        # args = {'language':'en','output':'outputto.mp3'})
    ...
    if not keepSrc:
        subprocess.call("rm {0}/{1}".format(SRCDIR, srcname),
                        shell=True)  # remove tar source file
        print("Removed {0}/{1}".format(SRCDIR, srcname))

    if not keepTex:
        subprocess.call("rm {0}/{1}".format(TMPDIR, fname),
                        shell=True)  # remove text file
        print("Removed {0}/{1}".format(TMPDIR, fname))

    if not saveText:
        subprocess.call("rm {0}/{1}".format(FINALDIR, output_txt),
                        shell=True)  # remove text file
        print("Removed {0}/{1}".format(FINALDIR, output_txt))

    if autoplay:
            """ Playing audio file just created """
            # possibly need different calls depending on the players if
            # they have different input params, i.e. for speed etc
            subprocess.call("{0} {1}/{2} {3}".format(player, FINALDIR,
                            output_audio, options), shell=True)
